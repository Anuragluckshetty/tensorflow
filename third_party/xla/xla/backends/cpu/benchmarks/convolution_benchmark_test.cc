...
static void BM_Conv2D(benchmark::State& state,
  ...
  int padding_mode = state.range(7);  // 1: SAME, 0: VALID
...
void AddConv2DArgs(::benchmark::internal::Benchmark* b, int type_config) {
  ...
  if (type_config != 2) {
    add_args({8, 5, 5, 1, 1, 1, 32, 1});
  }
  add_args({8, 5, 5, 4, 1, 1, 32, 1});
  add_args({8, 128, 128, 4, 1, 1, 8, 1});
  // Shapes from TF convolution benchmarks.
  add_args({8, 32, 32, 128, 1, 1, 1024, 1});
  add_args({16, 32, 32, 128, 1, 1, 1024, 1});
  add_args({32, 32, 32, 128, 1, 1, 1024, 1});
  // Shapes similar to Eigen spatial convolution benchmarks.
  add_args({32, 64, 64, 32, 1, 1, 64, 1});
  add_args({32, 256, 256, 4, 1, 1, 16, 1});
  add_args({32, 64, 64, 4, 1, 1, 16, 1});
  add_args({32, 32, 32, 96, 1, 1, 96, 1});
  // --------------------------------------------------------------------------
  // // 3x3 Convolution: SpatialConvolution
  // --------------------------------------------------------------------------
  // // Shapes from XLA convolution tests
  add_args({8, 5, 5, 1, 3, 3, 32, 1});
  add_args({8, 5, 5, 4, 3, 3, 32, 1});
  add_args({8, 128, 128, 4, 3, 3, 8, 1});
  // Shapes from TF convolution benchmarks
  add_args({8, 32, 32, 128, 3, 3, 1024, 1});
  add_args({16, 32, 32, 128, 3, 3, 1024, 1});
  add_args({32, 32, 32, 128, 3, 3, 1024, 1});
  // Shapes similar to Eigen spatial convolution benchmarks.
  // Same padding.
  add_args({32, 64, 64, 32, 3, 3, 64, 1});
  add_args({32, 256, 256, 4, 3, 3, 16, 1});
  add_args({32, 64, 64, 4, 3, 3, 16, 1});
  add_args({32, 32, 32, 96, 3, 3, 96, 1});
  // Valid padding.
  add_args({32, 64, 64, 32, 3, 3, 64, 0});
  add_args({32, 256, 256, 4, 3, 3, 16, 0});
  add_args({32, 64, 64, 4, 3, 3, 16, 0});
  add_args({32, 32, 32, 96, 3, 3, 96, 0});
}
